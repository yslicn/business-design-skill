...
例如某环节六维得分为 `5/5/4/4/5/4`，则市场吸引力贡献为 `5×0.20+5×0.20+4×0.15=2.60`，企业胜任权贡献为 `4×0.20+5×0.15+4×0.10=1.95`，总分为 `4.55`。Report consultant 在正文解释评分方法、关键差异和取舍，附录保存全部贡献、总分及评分理由；不能用总分图代替推理。

量化数据可来自可追溯研究、用户材料或已核实原始资料，登记来源、定位和口径。无法直接核实的数据须标注为估算，给出估算逻辑和证据编号；无法形成合理估算时标注 unknown，不得用虚假数字填满结构。
...
```
Sxx 来源 -> Exx 证据 -> 业务结论的 evidence_ids -> MD/DOCX 正文及展项的证据标记
```
...
- **正文证据标记**：MD 保留关键事实与结论的证据编号，DOCX 同步保留；附录呈现 `Exx -> Sxx` 映射。用户另需 PPT 时按下游交接契约处理。
